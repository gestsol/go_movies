defmodule GoMovie.Account do
  ...
  """
  def get_benefit_request!(id) do
    Repo.get!(BenefitRequest, id)
  end
  ...
  def delete_user_session(user_id, token) do
    from(us in UserSession, where: us.user_id == ^user_id and us.token == ^token)
    |> Repo.one!()
    |> delete_user_session()
  end
  ...
  def get_user_sessions_count(user_id) do
    from(
      us in UserSession,
      select: count("*"),
