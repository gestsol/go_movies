defmodule GoMovieWeb.MResourceMovieController do
  ...
  def validate_genders(genders) do
    ...
    error = cond do
      is_nil(genders) -> "Missing field genders."
      is_list(genders) == false -> "Field genders must be of type array."
      length(genders) == 0 -> "Field genders is empty."
      is_nil(Enum.find(genders, fn g -> is_binary(g) == false end)) == false -> "Field genders must be an array of strings"
      true -> nil
    end
